...
from aps.ml.iv_damage_model import CalibratedDamageModel, DamageRequest
...
def save_model_artifact(
    ...
    *,
    ...
    if destination.exists():
        raise DamageOperationError(f"artifact already exists and is immutable: {destination}")
...
SELECT model.id, model.model_version, model.release_status, model.stress_type,
       ...
       policy.approved,
       COALESCE((model.validation_metrics->>'release_gate_eligible')::boolean, false)
...
def release_model(
    ...
    *,
    ...
    try:
        ...
        _validate_releasable(candidate, allowed_statuses={ReleaseStatus.VALIDATED, ReleaseStatus.SHADOW})
        model_id, _, _, stress_type, target_type = candidate[:5]
        cursor.execute(
...
def score_request(
    ...
    missing = sorted(
        name for name in model.required_features if name not in features or features[name] is None
    )
    return ScoredPrediction(
        ...
        feature_completeness={"complete": not missing, "missing": missing},
    )
...
def score_pending_requests(conn, *, limit: int = 500) -> ScoreBatchResult:
    ...
    try:
        ...
        for row in rows:
            ...
            if model is None:
                model = load_model_artifact(resolve_artifact_path(row[14]), row[15])
...
def record_prediction_outcome(
    ...
    *,
    ...
    """Append an outcome only when it was measured after the prediction request."""
    ...
    try:
        ...
        if not all(row[3:7]):
            raise DamageOperationError(
                "outcome must match device/stress/target and be created after the request"
            )
        cursor.execute(
            """
            INSERT INTO iv_damage_prediction_outcomes (
                request_id, response_unit_id, observed_response,
                match_method, reviewed_by, review_notes
            ) VALUES (%s, %s, %s, %s, %s, %s)
            RETURNING id
            """,
            (row[0], row[1], row[2], match_method, reviewed_by, review_notes),
        )
